...
def get_parser():
    ...
    parser.add_argument(
        "--freezeBN",
        choices=dict(true=True, false=False),
        default=True,
...
def val_cihp(
    net_, testloader, testloader_flip, test_graph, epoch, writer, criterion, classes=20
):
    ...
    for ii, sample_batched in enumerate(zip(testloader, testloader_flip)):
        ...
        if ii % num_img_ts == num_img_ts - 1:
            ...
            print("Validation:")
            print(
                "[Epoch: %d, numImages: %5d]" % (epoch, ii * 1 + inputs.data.shape[0])
            )
...
def main(opts):
    ...
    voc_train = cihp.VOCSegmentation(
    ...
    voc_val_flip = cihp.VOCSegmentation(
    ...
    for epoch in range(resume_epoch, nEpochs):
        ...
        for ii, sample_batched in enumerate(trainloader):
            ...
            if aveGrad % p["nAveGrad"] == 0:
                writer.add_scalar(
                    "data/total_loss_iter", loss.item(), ii + num_img_tr * epoch
                )
                optimizer.step()
                optimizer.zero_grad()
                aveGrad = 0
